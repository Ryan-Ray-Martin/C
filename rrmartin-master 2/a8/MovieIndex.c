...
int AddMovieToIndex(Index index, Movie *movie, enum IndexField field) {
  ...
  if (result < 0) {
	...
	//snprintf(genres, sizeof(genres), "%s", movie->genres);
	// doc_set_name = genres;
	AddMovieToIndex_Genre(index, movie);
    }
  ...
  AddMovieToSet((MovieSet)kvp.value, kvp.key, field);
  // AddMovieTitleToIndex(kvp.value, movie, kvp.key, field);
