...
Index BuildMovieIndex(LinkedList movies, enum IndexField field_to_index){
  ...
  if (NumElemsInHashtable(movies) == 0) {
    DestroyHashtable((Hashtable) movies, &free);
    movies = NULL;
  }
  ...
  while (LLIterHasNext(iter)) {
    ...
    if (result == 2) {
      ...
      // We add a chain to the linked list in order to resolve collision. 
      InsertLinkedList((LinkedList) iter, &result);
    }
  }
